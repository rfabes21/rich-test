...
install-rich:
	git remote add rich git@github.com:blitzagency/rich.git
	git fetch rich
	git checkout -b rich rich/develop
	git subtree split -P rich --branch rich_lib
	git checkout master
	git subtree add --prefix src/static/js/vendor/famous/rich --squash rich_lib > /dev/null 2>&1


push-rich:
	git subtree push --prefix src/static/js/vendor/famous/rich --squash . rich_lib
	git subtree merge --prefix . rich_lib rich
	#git subtree split --prefix=src/static/js/vendor/famous/rich --onto rich --branch rich_lib
